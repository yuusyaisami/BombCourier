...
- GrainScale が極端で、粒子が見えにくくなっている。

### Grain Fix

- Medium 以上で確認します。
- GrainStrength を一時的に少し上げ、GrainScale は 1 前後から切り分けます。
- TemporalStrength は 0.1 前後を基準に、動きが気になるなら下げます。
...
- Mobile_Renderer 上で authored Quality Tier が Low 以外のため、resolved runtime tier と inspector summary がずれている。

### Mobile Warning Fix

- Mobile_Renderer.asset では Force Low Quality Tier を on にします。
- selected preset を MobileOptimized に戻します。
- authored Quality Tier も Low に戻して、resolved runtime tier と inspector summary を揃えます。
- mobile path は Low runtime budget 固定が前提なので、Medium 以上を authoring で期待しません。

## Renderer Policy Build Fails

### Renderer Policy Failure Symptoms

- build 開始時に ToyDiorama renderer policy error で停止する。

### Renderer Policy Failure Cause

- PC_Renderer 上の ToyDiorama feature で Force Low Quality Tier が on。
- Mobile_Renderer 上の ToyDiorama feature が inactive。
- Mobile_Renderer 上の settings.Enabled が off。
- Mobile_Renderer 上の Force Low Quality Tier が off。

### Renderer Policy Failure Fix

- PC_Renderer では Force Low Quality Tier を off に戻します。
- Mobile_Renderer では feature を active、settings.Enabled を on、Force Low Quality Tier を on に戻します。
- mobile authoring は MobileOptimized preset と Low tier を starting point に戻します。
