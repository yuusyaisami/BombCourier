...
Shader "Universal Render Pipeline/RealToon/Version 5/Default/Default"
{
    Properties
    {
		...
		[Enum(On, 1, Off, 0)] _OutZWrite("ZWrite", int) = 1
		[Enum(UnityEngine.Rendering.CompareFunction)] _OutZTest("ZTest", int) = 4
		...
		_PSGLOTEX ("Pattern Style", Float ) = 0.0
        ...
        _SelfShadowShadowTAtViewDirection ("Self Shadow & ShadowT At View Direction", Float ) = 0.0
		...
		_LigIgnoYNorDir ("Light Ignore Y Normal Direction", Float) = 0
		...
		_TessellationSmoothness ("Smoothness", Range(0, 2)) = 0.5
		...
		[RTToggleOption] _NorMapAsDis ("Normal Map As Displacement", Float ) = 0
...
//TessTess_FP_1_End*/
		...
		#pragma shader_feature_local N_F_DDMD_ON
...
Varyings LitPassVertex(Attributes input)//FPV
        {
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_FL
			...
			//RT_PA
			#if N_F_PA_ON
				output.positionCS = mul(RT_PA(-vertexInput.positionVS.z), float4(_LBS_CD_Position.xyz,1.0) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
			#else
			...
			//
				...
				output.uv = input.uv;
			#elif _UVSET_UV1
				output.uv = input.staticLightmapUV;
			#endif
...
/*//Tess_FP_2
		...
		TessVertex LitPassVertex(Attributes v)
		{
		...
		float Tessellation(TessVertex v)
		{
			...
			half RTD_TES = lerp(_TessellationNear,_TessellationFar,saturate( ( distance(mul(GetObjectToWorldMatrix(), v.positionOS).xyz,_WorldSpaceCameraPos) * RTD_TES_TRANS_Sli ) ) );
		...
		[domain("tri")]
		Varyings domain (OutputPatchConstant tessFactors, const OutputPatch<TessVertex,3> vi, float3 bary : SV_DomainLocation) 
		{
			...
			v.positionOS.xyz = _TessellationSmoothness * (pp[0] * bary.x + pp[1] * bary.y + pp[2] * bary.z) + (1.0f - _TessellationSmoothness) * v.positionOS.xyz;
			v.normalOS = normalize( (vi[0]. normalOS * bary.x + vi[1].normalOS * bary.y + vi[2].normalOS * bary.z) );
			...
			if (_NorMapAsDis == 1)
			{
				float2 uv = TRANSFORM_TEX(v.uv, _NormalMap);
				float norma_disp = dot(float3(0.299, 0.587, 0), normalize(UnpackNormal(SAMPLE_TEXTURE2D_LOD(_NormalMap, sampler_NormalMap, uv, 0))) ) ;
				v.positionOS.xyz += v.normalOS * (norma_disp * (_NormalMapIntensity * 0.01) );
			}

			Varyings o = PostProcessVertex(v);
			return o;
		...
		) 
        {
			...
			//**
				...
				half4 _MainTex_var = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, TRANSFORM_TEX(RTD_TC_TP_OO, _MainTex));
					...
					RT_DC(input.positionCS, _MainTex_var, normalDirection); //cc
				#endif
			#endif
...
Varyings ShadowPassVertex(Attributes input)//SCP
		{
...
/*//Tess_SCP_2
		...
		TessVertex ShadowPassVertex(Attributes v)
		{
			TessVertex o = (TessVertex)0;
			UNITY_SETUP_INSTANCE_ID(v);
			UNITY_TRANSFER_INSTANCE_ID(v, o);
			...
			#if defined(UNITY_DOTS_INSTANCING_ENABLED) && !defined(N_F_DDMD_ON)
				o.tangentOS = v.tangentOS;
			#endif
		...
		Varyings domain (OutputPatchConstant tessFactors, const OutputPatch<TessVertex,3> vi, float3 bary : SV_DomainLocation) 
		{
			...
			if (_NorMapAsDis == 1)
			{
				float2 uv = TRANSFORM_TEX(v.texcoord, _NormalMap);
				float norma_disp = dot(float3(0.299, 0.587, 0), normalize(UnpackNormal(SAMPLE_TEXTURE2D_LOD(_NormalMap, sampler_NormalMap, uv, 0))) ) ;
				v.positionOS.xyz += v.normalOS * (norma_disp * (_NormalMapIntensity * 0.01) );
			}
...
Varyings LitPassVertex(Attributes input)//GBP
        {
...
//Tess_GBP_2_End*/
        ...
        void LitPassFragment(
							...
							, float facing : VFACE
						#ifdef _WRITE_RENDERING_LAYERS
							, out uint outRenderingLayers : SV_Target5
						#endif
		) 
        {
...
Varyings DepthOnlyVertex(Attributes input)//DOP
		{
			Varyings output = (Varyings)0;
			UNITY_SETUP_INSTANCE_ID(input);
			UNITY_TRANSFER_INSTANCE_ID(input, output);
...
/*//Tess_DOP_2
		...
		TessVertex DepthOnlyVertex(Attributes v)
		{
			TessVertex o = (TessVertex)0;
			UNITY_SETUP_INSTANCE_ID(v);
			UNITY_TRANSFER_INSTANCE_ID(v, o);
		...
		Varyings domain (OutputPatchConstant tessFactors, const OutputPatch<TessVertex,3> vi, float3 bary : SV_DomainLocation) 
		{
			...
			v.position = vi[0].positionOS*bary.x + vi[1].positionOS*bary.y + vi[2].positionOS*bary.z;

			float3 pp[3];
			for (int i = 0; i < 3; ++i)
			{
				pp[i] = v.position.xyz - vi[i].normalOS * (dot(v.position.xyz, vi[i].normalOS) - dot(vi[i].positionOS.xyz, vi[i].normalOS));
			}

			v.position.xyz = _TessellationSmoothness * (pp[0] * bary.x + pp[1] * bary.y + pp[2] * bary.z) + (1.0f - _TessellationSmoothness) * v.position.xyz;
			v.normalOS = normalize( (vi[0]. normalOS * bary.x + vi[1].normalOS * bary.y + vi[2].normalOS * bary.z) );
			v.tangentOS = vi[0].tangentOS*bary.x + vi[1].tangentOS*bary.y + vi[2].tangentOS*bary.z;
			v.texcoord = vi[0].uv*bary.x + vi[1].uv*bary.y + vi[2].uv*bary.z;
			...
			if (_NorMapAsDis == 1)
			{
				float2 uv = TRANSFORM_TEX(v.texcoord, _NormalMap);
				float norma_disp = dot(float3(0.299, 0.587, 0), normalize(UnpackNormal(SAMPLE_TEXTURE2D_LOD(_NormalMap, sampler_NormalMap, uv, 0))) ) ;
				v.position.xyz += v.normalOS * (norma_disp * (_NormalMapIntensity * 0.01) );
			}
...
Varyings DepthNormalsVertex(Attributes input)//DNP
		{
			Varyings output = (Varyings)0;
			UNITY_SETUP_INSTANCE_ID(input);
			UNITY_TRANSFER_INSTANCE_ID(input, output);
...
/*//Tess_DNP_2
		...
		TessVertex DepthNormalsVertex(Attributes v)
		{
			TessVertex o = (TessVertex)0;
			UNITY_SETUP_INSTANCE_ID(v); 
			UNITY_TRANSFER_INSTANCE_ID(v, o);
		...
		Varyings domain (OutputPatchConstant tessFactors, const OutputPatch<TessVertex,3> vi, float3 bary : SV_DomainLocation) 
		{
			...
			v.positionOS.xyz = _TessellationSmoothness * (pp[0] * bary.x + pp[1] * bary.y + pp[2] * bary.z) + (1.0f - _TessellationSmoothness) * v.positionOS.xyz;
			v.normal = normalize( (vi[0]. normalOS * bary.x + vi[1].normalOS * bary.y + vi[2].normalOS * bary.z) );
			v.tangentOS = vi[0].tangentOS*bary.x + vi[1].tangentOS*bary.y + vi[2].tangentOS*bary.z;
			v.texcoord = vi[0].uv*bary.x + vi[1].uv*bary.y + vi[2].uv*bary.z;
			...
			if (_NorMapAsDis == 1)
			{
				float2 uv = TRANSFORM_TEX(v.texcoord, _NormalMap);
				float norma_disp = dot(float3(0.299, 0.587, 0), normalize(UnpackNormal(SAMPLE_TEXTURE2D_LOD(_NormalMap, sampler_NormalMap, uv, 0))) ) ;
				v.positionOS.xyz += v.normal * (norma_disp * (_NormalMapIntensity * 0.01) );
			}
...
#pragma target 2.0 //targetol
        ...
        #pragma multi_compile_instancing
		#pragma instancing_options renderinglayer
		#include_with_pragmas "Packages/com.unity.render-pipelines.universal/ShaderLibrary/DOTS.hlsl" //cc

		#pragma multi_compile_fragment _ LOD_FADE_CROSSFADE

//#define N_F_TESS_ON//OT
...
Varyings LitPassVertex(Attributes input)//OP
        {
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_OL
			...
			//RT_PA
			#if N_F_PA_ON
				output.positionCS = mul(RT_PA(-vertexInput.positionVS.z), float4( (_LBS_CD_Position.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL, 1.0) ) + (float4(0,0,_ObjePosiZCS,0.0) * 0.0001);
			#else
